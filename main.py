...
def main():
    ...
    if "voice_pipeline" not in st.session_state:
        try:
            ...
            llm_coach = LLMCoach(anthropic_client)
            tts = TextToSpeech()
    ...
    with st.sidebar:
        ...
        if not workout_started:
            ...
            if end_session_button:
                ...
                if sets_completed > last_saved and reps_per_set > 0 and exercise:
                    newly_completed = sets_completed - last_saved
                    now_ts = time.time()
                    started_at = st.session_state.get("set_cycle_started_at", now_ts)
                    add_exercise(user_id, exercise, newly_completed * reps_per_set, newly_completed, now_ts - started_at)
